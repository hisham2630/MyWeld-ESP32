...
static const weld_preset_t factory_presets[MAX_PRESETS] = {
    ...
    { "Stainless Steel", 20.0f, 40.0f, 30.0f,  0.0f, 0.0f },  // 5
    { "Aluminum",        25.0f, 45.0f, 35.0f,  0.0f, 0.0f },  // 6 (last factory)
    { "Custom 1",        10.0f, 25.0f, 10.0f,  0.0f, 0.0f },  // 7
    { "Custom 2",        10.0f, 25.0f, 10.0f,  0.0f, 0.0f },  // 8
    { "Custom 3",        10.0f, 25.0f, 10.0f,  0.0f, 0.0f },  // 9
    { "Custom 4",        10.0f, 25.0f, 10.0f,  0.0f, 0.0f },  // 10
    { "Custom 5",        10.0f, 25.0f, 10.0f,  0.0f, 0.0f },  // 11
    { "Custom 6",        10.0f, 25.0f, 10.0f,  0.0f, 0.0f },  // 12
    { "Custom 7",        10.0f, 25.0f, 10.0f,  0.0f, 0.0f },  // 13
    { "Custom 8",        10.0f, 25.0f, 10.0f,  0.0f, 0.0f },  // 14
    { "Custom 9",        10.0f, 25.0f, 10.0f,  0.0f, 0.0f },  // 15
    { "Custom 10",       10.0f, 25.0f, 10.0f,  0.0f, 0.0f },  // 16
    { "Custom 11",       10.0f, 25.0f, 10.0f,  0.0f, 0.0f },  // 17
    { "Custom 12",       10.0f, 25.0f, 10.0f,  0.0f, 0.0f },  // 18
    { "Custom 13",       10.0f, 25.0f, 10.0f,  0.0f, 0.0f },  // 19
};
...
static void settings_load_defaults(void)
{
    ...
    g_settings.p3 = PULSE_DEFAULT_P3;
    g_settings.p4 = PULSE_DEFAULT_P4;
...
static void settings_load_from_nvs(void)
{
    ...
    g_settings.p3 = nvs_get_float("p3", PULSE_DEFAULT_P3);
    g_settings.p4 = nvs_get_float("p4", PULSE_DEFAULT_P4);
    ...
    ESP_LOGI(TAG, "Settings loaded: P1=%.1f T=%.1f P2=%.1f P3=%.1f P4=%.1f mode=%s preset=%d welds=%lu",
...
static void settings_write_to_nvs(void)
{
    ...
    nvs_set_float("p3", g_settings.p3);
    nvs_set_float("p4", g_settings.p4);
...
{
    if (index == PRESET_USER_DEFINED || index >= MAX_PRESETS) return;
    ...
    // Reset welding trigger flags so the new parameters take effect cleanly.
    // Without this, s_auto_fired / s_man_fired can stay stuck from a previous
    // weld cycle, making the trigger path appear dead after a preset change.
    welding_reset_state();

    ESP_LOGI(TAG, "Loaded preset %d: %s (P1=%.1f T=%.1f P2=%.1f P3=%.1f P4=%.1f) [mode=%s]",
             index, p->name, p->p1, p->t, p->p2, p->p3, p->p4,
             g_settings.auto_mode ? "AUTO" : "MAN");
}
...
void settings_set_user_defined(void)
{
    ...
    settings_save();
    welding_reset_state();
    ESP_LOGI(TAG, "Switched to User Defined (P1=%.1f T=%.1f P2=%.1f P3=%.1f P4=%.1f S=%.1f)",
             g_settings.p1, g_settings.t, g_settings.p2, g_settings.p3, g_settings.p4,
             g_settings.s_value);
}
