...
typedef struct __attribute__((packed)) {
    ...
    // Debug / calibration data (bytes 36–43)
...
} ble_params_packet_t;          // Total: 33 bytes
 ...
 * Sent in response to BLE_MSG_PRESET_LIST request (firmware sends 4 notifications).
 *
 * Payload layout per packet:
 *   [0]      page_index  (0–3)
 *   Per preset (5 per page):
 *     [20 bytes] name  (null-terminated, zero-padded)
 *     [2 bytes]  p1_x10  (little-endian)
 *     [2 bytes]  t_x10   (little-endian)
 *     [2 bytes]  p2_x10  (little-endian)
 *     [2 bytes]  p3_x10  (little-endian)
 *     [2 bytes]  p4_x10  (little-endian)
 *   = 30 bytes per preset
 *
 * Total: 1 + 5 × 30 = 151 bytes per page, 4 pages → 20 presets.
 * 151 bytes fits in the 1-byte LEN field (max 255) and one BLE MTU frame.
 */
#define BLE_PRESET_NAME_LEN     20
#define BLE_MAX_PRESETS         20
#define BLE_PRESETS_PER_PAGE    5    // Presets per PRESET_LIST_RESP packet
#define BLE_PRESET_ENTRY_SIZE   30   // 20 (name) + 5×2 (params) = 30 bytes

typedef struct __attribute__((packed)) {
    uint8_t page_index;                                    // 0–3
    struct __attribute__((packed)) {
        char     name[BLE_PRESET_NAME_LEN]; // 20 bytes
        uint16_t p1_x10;                   // P1 in 0.1ms units
        uint16_t t_x10;                    // T  in 0.1ms units
        uint16_t p2_x10;                   // P2 in 0.1ms units
        uint16_t p3_x10;                   // P3 in 0.1ms units
        uint16_t p4_x10;                   // P4 in 0.1ms units
    } entries[BLE_PRESETS_PER_PAGE];        // 5 × 30 = 150 bytes
} ble_preset_list_resp_t;                  // Total payload: 151 bytes
