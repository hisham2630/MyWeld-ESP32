 ...
 *   IDLE → ARMED → PRE_FIRE → FIRING_P1 → PAUSE_1 → FIRING_P2 → PAUSE_2 →
 *          FIRING_P3 → PAUSE_3 → FIRING_P4 → COOLDOWN → IDLE
...
// Post-weld retrigger lockout: blocks new MAN triggers for MAN_RETRIGGER_LOCKOUT_MS
// after firing, to prevent EMI from the weld pulse causing false re-triggers
static int64_t s_man_lockout_us = 0;
...
void welding_reset_state(void)
{
    ...
    // If we were armed (waiting for S delay), drop back to IDLE
    if (g_weld_state == WELD_STATE_ARMED) {
        g_weld_state = WELD_STATE_IDLE;
        ui_update_weld_state(WELD_STATE_IDLE);
    }
    ESP_LOGI(TAG, "Welding state reset (parameter change)");
}
...
{
    // Safety: refuse to fire if P1 is effectively zero (< 1ms)
    // NOTE: PULSE_MIN_MS (5ms) is the UI slider minimum, but factory presets
    // for thin materials (e.g. 0.1mm nickel) may use shorter pulses (3ms).
    // Using 1ms as the absolute hardware floor.
    if (g_settings.p1 < 1.0f) {
        ESP_LOGW(TAG, "FIRE aborted: P1=%.1fms is below 1ms minimum", g_settings.p1);
        return;
    }

    ESP_LOGI(TAG, "⚡ FIRING: P1=%.1f T=%.1f P2=%.1f P3=%.1f P4=%.1f ms",
    ...
    float snap_p3 = g_settings.p3;
    float snap_p4 = g_settings.p4;
    ...
    // 3. Pulse 2 (if P2 > 0 and T > 0)
    if (snap_p2 > 0.0f && snap_t > 0.0f) {
        g_weld_state = WELD_STATE_PAUSE_1;
        ...
        // 4. Pulse 3 (if P3 > 0)
        if (snap_p3 > 0.0f) {
            g_weld_state = WELD_STATE_PAUSE_2;
            ets_delay_us((uint32_t)(snap_t * 1000.0f));

            g_weld_state = WELD_STATE_FIRING_P3;
            portDISABLE_INTERRUPTS();
            gpio_set_level(PIN_OUTPUT, 1);
            ets_delay_us((uint32_t)(snap_p3 * 1000.0f));
            gpio_set_level(PIN_OUTPUT, 0);
            portENABLE_INTERRUPTS();

            // 5. Pulse 4 (if P4 > 0)
            if (snap_p4 > 0.0f) {
                g_weld_state = WELD_STATE_PAUSE_3;
                ets_delay_us((uint32_t)(snap_t * 1000.0f));

                g_weld_state = WELD_STATE_FIRING_P4;
                portDISABLE_INTERRUPTS();
                gpio_set_level(PIN_OUTPUT, 1);
                ets_delay_us((uint32_t)(snap_p4 * 1000.0f));
                gpio_set_level(PIN_OUTPUT, 0);
                portENABLE_INTERRUPTS();
            }
        }
    ...
    // 6. Start charger hold-off timer (charger stays DISABLED)
    ...
    while (1) {
        ...
        if (g_weld_state == WELD_STATE_IDLE || g_weld_state == WELD_STATE_ARMED) {
            ...
            if (!g_settings.auto_mode) {
                // Post-weld lockout: skip all trigger processing during cooldown
                // This prevents EMI noise from the weld pulse from causing
                // false "release → press" transitions on the trigger pin
                if (s_man_lockout_us > 0) {
                    int64_t elapsed = esp_timer_get_time() - s_man_lockout_us;
                    if (elapsed >= (int64_t)MAN_RETRIGGER_LOCKOUT_MS * 1000LL) {
                        s_man_lockout_us = 0;
                        // Re-read current pin state as baseline after lockout
                        s_start_btn_prev = (gpio_get_level(PIN_START) == 0);
                        s_debounce_count = 0;
                        s_man_fired = s_start_btn_prev; // If still held, stay locked
                        ESP_LOGI(TAG, "MAN: Retrigger lockout expired, pin=%d",
                                 s_start_btn_prev);
                    }
                    // Skip trigger logic while locked out
                } else {
                            ...
                            if (btn_pressed && !was_pressed) {
                                ...
                                if (!s_man_fired) { // One-shot: ignore if already fired this press
                                    if (!g_weld_status.low_voltage_block && !g_weld_status.protection_fault) {
                                        ...
                                        ESP_LOGI(TAG, "MAN: Weld fired, lockout %dms",
                                                 MAN_RETRIGGER_LOCKOUT_MS);
                                    } else {
                                        audio_play_error();
...
const char* welding_state_str(weld_state_t state)
{
    switch (state) {
        ...
        case WELD_STATE_PAUSE_1:    return "PAUSE 1";
        case WELD_STATE_FIRING_P2:  return "FIRING P2";
        case WELD_STATE_PAUSE_2:    return "PAUSE 2";
        case WELD_STATE_FIRING_P3:  return "FIRING P3";
        case WELD_STATE_PAUSE_3:    return "PAUSE 3";
        case WELD_STATE_FIRING_P4:  return "FIRING P4";
