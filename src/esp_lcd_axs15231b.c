...
static esp_err_t panel_axs15231b_invert_color(esp_lcd_panel_t *panel,
                                              bool invert_color_data);
static esp_err_t panel_axs15231b_mirror(esp_lcd_panel_t *panel, bool mirror_x,
                                        bool mirror_y);
static esp_err_t panel_axs15231b_swap_xy(esp_lcd_panel_t *panel,
                                         bool swap_axes);
static esp_err_t panel_axs15231b_set_gap(esp_lcd_panel_t *panel, int x_gap,
                                         int y_gap);
...
static bool touch_axs15231b_get_xy(esp_lcd_touch_handle_t tp, uint16_t *x,
...
static esp_err_t i2c_read_bytes(esp_lcd_touch_handle_t tp, int reg,
                                uint8_t *data, uint8_t len);
static esp_err_t i2c_write_bytes(esp_lcd_touch_handle_t tp, int reg,
...
esp_err_t
esp_lcd_new_panel_axs15231b(const esp_lcd_panel_io_handle_t io,
  ...
  ESP_GOTO_ON_FALSE(axs15231b, ESP_ERR_NO_MEM, err, TAG,
                    "no mem for axs15231b panel");
  ...
  if (panel_dev_config->vendor_config) {
    axs15231b->init_cmds =
        ((axs15231b_vendor_config_t *)panel_dev_config->vendor_config)
            ->init_cmds;
    axs15231b->init_cmds_size =
        ((axs15231b_vendor_config_t *)panel_dev_config->vendor_config)
            ->init_cmds_size;
    axs15231b->flags.use_qspi_interface =
        ((axs15231b_vendor_config_t *)panel_dev_config->vendor_config)
            ->flags.use_qspi_interface;
  }
  ...
  *ret_panel = &(axs15231b->base);
  ...
  ESP_LOGI(TAG, "LCD panel create success, version: %d.%d.%d",
           ESP_LCD_AXS15231B_VER_MAJOR, ESP_LCD_AXS15231B_VER_MINOR,
...
static esp_err_t tx_param(axs15231b_panel_t *axs15231b,
...
static esp_err_t tx_color(axs15231b_panel_t *axs15231b,
                          esp_lcd_panel_io_handle_t io, int lcd_cmd,
                          const void *param, size_t param_size) {
  if (axs15231b->flags.use_qspi_interface) {
    lcd_cmd &= 0xff;
...
static esp_err_t panel_axs15231b_reset(esp_lcd_panel_t *panel) {
  ...
  if (axs15231b->reset_gpio_num >= 0) {
    ...
    vTaskDelay(pdMS_TO_TICKS(
        120)); // spec, wait at least 5m before sending new command
  }
...
static const axs15231b_lcd_init_cmd_t vendor_specific_init_default[] = {
    ...
    {0xA0,
     (uint8_t[]){0x00, 0x10, 0x00, 0x02, 0x00, 0x00, 0x64, 0x3F, 0x20, 0x05,
                 0x3F, 0x3F, 0x00, 0x00, 0x00, 0x00, 0x00},
     17, 0},
    {0xA2, (uint8_t[]){0x30, 0x04, 0x0A, 0x3C, 0xEC, 0x54, 0xC4, 0x30,
                       0xAC, 0x28, 0x7F, 0x7F, 0x7F, 0x20, 0xF8, 0x10,
                       0x02, 0xFF, 0xFF, 0xF0, 0x90, 0x01, 0x32, 0xA0,
                       0x91, 0xC0, 0x20, 0x7F, 0xFF, 0x00, 0x54},
     31, 0},
    {0xD0,
     (uint8_t[]){0x30, 0xAC, 0x21, 0x24, 0x08, 0x09, 0x10, 0x01, 0xAA, 0x14,
                 0xC2, 0x00, 0x22, 0x22, 0xAA, 0x03, 0x10, 0x12, 0x40, 0x14,
                 0x1E, 0x51, 0x15, 0x00, 0x40, 0x10, 0x00, 0x03, 0x3D, 0x12},
     30, 0},
    {0xA3, (uint8_t[]){0xA0, 0x06, 0xAA, 0x08, 0x08, 0x02, 0x0A, 0x04,
                       0x04, 0x04, 0x04, 0x04, 0x04, 0x04, 0x04, 0x04,
                       0x04, 0x04, 0x04, 0x00, 0x55, 0x55},
     22, 0},
    {0xC1,
     (uint8_t[]){0x33, 0x04, 0x02, 0x02, 0x71, 0x05, 0x24, 0x55, 0x02, 0x00,
    ...
    {0xC3,
     ...
     11, 0},
    {0xC4,
     (uint8_t[]){0x00, 0x24, 0x33, 0x90, 0x50, 0xea, 0x64, 0x32, 0xC8, 0x64,
                 0xC8, 0x32, 0x90, 0x90, 0x11, 0x06, 0xDC, 0xFA, 0x04, 0x03,
                 0x80, 0xFE, 0x10, 0x10, 0x00, 0x0A, 0x0A, 0x44, 0x50},
     29, 0},
    {0xC5, (uint8_t[]){0x18, 0x00, 0x00, 0x03, 0xFE, 0x78, 0x33, 0x20,
                       0x30, 0x10, 0x88, 0xDE, 0x0D, 0x08, 0x0F, 0x0F,
                       0x01, 0x78, 0x33, 0x20, 0x10, 0x10, 0x80},
     23, 0},
    {0xC6,
    ...
    {0xCF, (uint8_t[]){0x34, 0x1E, 0x88, 0x58, 0x13, 0x18, 0x56, 0x18, 0x1E,
                       0x68, 0xF7, 0x00, 0x65, 0x0C, 0x22, 0xC4, 0x0C, 0x77,
                       0x22, 0x44, 0xAA, 0x55, 0x04, 0x04, 0x12, 0xA0, 0x08},
     27, 0},
    {0xD5,
     (uint8_t[]){0x3E, 0x3E, 0x88, 0x00, 0x44, 0x04, 0x78, 0x33, 0x20, 0x78,
                 0x33, 0x20, 0x04, 0x28, 0xD3, 0x47, 0x03, 0x03, 0x03, 0x03,
                 0x86, 0x00, 0x00, 0x00, 0x30, 0x52, 0x3f, 0x40, 0x40, 0x96},
     30, 0},
    {0xD6,
     (uint8_t[]){0x10, 0x32, 0x54, 0x76, 0x98, 0xBA, 0xDC, 0xFE, 0x95, 0x00,
                 0x01, 0x83, 0x75, 0x36, 0x20, 0x75, 0x36, 0x20, 0x3F, 0x03,
                 0x03, 0x03, 0x10, 0x10, 0x00, 0x04, 0x51, 0x20, 0x01, 0x00},
     30, 0},
    {0xD7,
     (uint8_t[]){0x0a, 0x08, 0x0e, 0x0c, 0x1E, 0x18, 0x19, 0x1F, 0x00, 0x1F,
                 0x1A, 0x1F, 0x3E, 0x3E, 0x04, 0x00, 0x1F, 0x1F, 0x1F},
     19, 0},
    {0xD8,
     (uint8_t[]){0x0B, 0x09, 0x0F, 0x0D, 0x1E, 0x18, 0x19, 0x1F, 0x01, 0x1F,
                 0x1A, 0x1F},
     12, 0},
    {0xD9,
     (uint8_t[]){0x00, 0x0D, 0x0F, 0x09, 0x0B, 0x1F, 0x18, 0x19, 0x1F, 0x01,
                 0x1E, 0x1A, 0x1F},
     13, 0},
    {0xDD,
     (uint8_t[]){0x0C, 0x0E, 0x08, 0x0A, 0x1F, 0x18, 0x19, 0x1F, 0x00, 0x1E,
                 0x1A, 0x1F},
     12, 0},
    {0xDF, (uint8_t[]){0x44, 0x73, 0x4B, 0x69, 0x00, 0x0A, 0x02, 0x90}, 8, 0},
    {0xE0,
     (uint8_t[]){0x19, 0x20, 0x0A, 0x13, 0x0E, 0x09, 0x12, 0x28, 0xD4, 0x24,
                 0x0C, 0x35, 0x13, 0x31, 0x36, 0x2f, 0x03},
     17, 0},
    {0xE1,
     (uint8_t[]){0x38, 0x20, 0x09, 0x12, 0x0E, 0x08, 0x12, 0x28, 0xC5, 0x24,
                 0x0C, 0x34, 0x12, 0x31, 0x36, 0x2f, 0x27},
     17, 0},
    {0xE2,
     (uint8_t[]){0x19, 0x20, 0x0A, 0x11, 0x09, 0x06, 0x11, 0x25, 0xD4, 0x22,
                 0x0B, 0x33, 0x12, 0x2D, 0x32, 0x2f, 0x03},
     17, 0},
    {0xE3,
     (uint8_t[]){0x38, 0x20, 0x0A, 0x11, 0x09, 0x06, 0x11, 0x25, 0xC4, 0x21,
                 0x0A, 0x32, 0x11, 0x2C, 0x32, 0x2f, 0x27},
     17, 0},
    {0xE4,
     (uint8_t[]){0x19, 0x20, 0x0D, 0x14, 0x0D, 0x08, 0x12, 0x2A, 0xD4, 0x26,
                 0x0E, 0x35, 0x13, 0x34, 0x39, 0x2f, 0x03},
     17, 0},
    {0xE5,
     (uint8_t[]){0x38, 0x20, 0x0D, 0x13, 0x0D, 0x07, 0x12, 0x29, 0xC4, 0x25,
                 0x0D, 0x35, 0x12, 0x33, 0x39, 0x2f, 0x27},
     17, 0},
...
static esp_err_t panel_axs15231b_init(esp_lcd_panel_t *panel) {
  ...
  ESP_RETURN_ON_ERROR(tx_param(axs15231b, io, LCD_CMD_SLPOUT, NULL, 0), TAG,
                      "send command failed");
  ...
  for (int i = 0; i < init_cmds_size; i++) {
    ...
    ESP_RETURN_ON_ERROR(tx_param(axs15231b, io, init_cmds[i].cmd,
...
static esp_err_t panel_axs15231b_draw_bitmap(esp_lcd_panel_t *panel,
  ...
  // RASET must be sent in both SPI and QSPI modes.
  // Without it, the row address window is undefined and pixels land at
  // the wrong GRAM rows — causing a blank / corrupted screen in QSPI mode.
  ...
  size_t len =
      (x_end - x_start) * (y_end - y_start) * axs15231b->fb_bits_per_pixel / 8;
  if (y_start == 0) {
...
static esp_err_t panel_axs15231b_mirror(esp_lcd_panel_t *panel, bool mirror_x,
  ...
  tx_param(axs15231b, io, LCD_CMD_MADCTL, (uint8_t[]){axs15231b->madctl_val},
           1);
  return ESP_OK;
}
  ...
  if (axs15231b->config.int_gpio_num != GPIO_NUM_NC) {
    ...
    if (axs15231b->config.interrupt_callback) {
      esp_lcd_touch_register_interrupt_callback(
          axs15231b, axs15231b->config.interrupt_callback);
    }
  }
...
static esp_err_t touch_axs15231b_read_data(esp_lcd_touch_handle_t tp) {
  ...
  // AXS15231B does NOT respond to I2C when no finger is present — this is
  // normal. Use silent checks (no ESP_LOGE) to avoid flooding the UART with
  // spurious errors.
  esp_err_t ret = i2c_write_bytes(tp, -1, read_cmd, sizeof(read_cmd));
  if (ret != ESP_OK) {
    return ret; // silent — no touch event, perfectly normal
  }
  ret = i2c_read_bytes(tp, -1, data, sizeof(data));
  if (ret != ESP_OK) {
    return ret; // silent
  }

  touch_record_struct_t *p_touch_data = (touch_record_struct_t *)data;
  if (p_touch_data->num && (AXS_MAX_TOUCH_NUMBER >= p_touch_data->num)) {
    ...
    for (int i = 0; i < tp->data.points; i++) {
      tp->data.coords[i].x =
          ((p_touch_data->x_h & 0x0F) << 8) | p_touch_data->x_l;
      tp->data.coords[i].y =
          ((p_touch_data->y_h & 0x0F) << 8) | p_touch_data->y_l;
    }
