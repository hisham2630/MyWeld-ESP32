...
static uint8_t map_weld_state(weld_state_t state)
{
    switch (state) {
        ...
        case WELD_STATE_PAUSE_1:
        case WELD_STATE_FIRING_P2:
        case WELD_STATE_PAUSE_2:
        case WELD_STATE_FIRING_P3:
        case WELD_STATE_PAUSE_3:
        case WELD_STATE_FIRING_P4: return 3; // FIRING
...
static void build_status_payload(ble_status_packet_t *pkt)
{
    ...
    pkt->p3_x10         = (uint16_t)(g_settings.p3 * 10.0f);
    pkt->p4_x10         = (uint16_t)(g_settings.p4 * 10.0f);
                            ...
                            struct ble_gatt_access_ctxt *ctxt, void *arg)
{
    if (ctxt->op == BLE_GATT_ACCESS_OP_READ_CHR) {
        ...
        params.p3_x10    = (uint16_t)(g_settings.p3 * 10.0f);
        params.p4_x10    = (uint16_t)(g_settings.p4 * 10.0f);
    ...
    if (ctxt->op == BLE_GATT_ACCESS_OP_WRITE_CHR) {
        ...
        float new_p3 = p->p3_x10 / 10.0f;
        float new_p4 = p->p4_x10 / 10.0f;
        ...
        // T can be 0 (single-pulse) or PAUSE_MIN..PAUSE_MAX
        if (new_t != 0.0f && (new_t < PAUSE_MIN_MS || new_t > PAUSE_MAX_MS)) {
            ESP_LOGW(TAG, "T out of range: %.1f", new_t);
            send_nak(BLE_MSG_PARAMS_WRITE, BLE_ERR_INVALID_RANGE);
            return 0;
        }
        ...
        if (new_s < S_VALUE_MIN || new_s > S_VALUE_MAX) {
            send_nak(BLE_MSG_PARAMS_WRITE, BLE_ERR_INVALID_RANGE);
            return 0;
        }

        // Enforce P1 ≤ P2 rule: warm-up pulse must not exceed main pulse
        if (new_p2 > 0.0f && new_p1 > new_p2) {
            new_p1 = new_p2;
            ESP_LOGW(TAG, "P1 clamped to P2 (%.1f) — warm-up > main is invalid", new_p2);
        }
        ...
        g_settings.p3        = new_p3;
        g_settings.p4        = new_p4;
                         ...
                         struct ble_gatt_access_ctxt *ctxt, void *arg)
{
    ...
    if (msg_type == BLE_MSG_PRESET_LIST) {
        // Authentication required to read preset data
        if (!s_authenticated) {
            send_nak(BLE_MSG_PRESET_LIST, BLE_ERR_AUTH_REQUIRED);
            return 0;
        }
        // Send full preset data in 4 paged notifications (5 presets × 30 bytes each = 151b/page)
        uint8_t total_pages = BLE_MAX_PRESETS / BLE_PRESETS_PER_PAGE;
        for (uint8_t page = 0; page < total_pages; page++) {
            ...
            for (int i = 0; i < BLE_PRESETS_PER_PAGE && (base + i) < MAX_PRESETS; i++) {
                const weld_preset_t *src = &g_settings.presets[base + i];
                strncpy(resp.entries[i].name, src->name, BLE_PRESET_NAME_LEN - 1);
                resp.entries[i].p1_x10 = (uint16_t)(src->p1 * 10.0f);
                resp.entries[i].t_x10  = (uint16_t)(src->t  * 10.0f);
                resp.entries[i].p2_x10 = (uint16_t)(src->p2 * 10.0f);
                resp.entries[i].p3_x10 = (uint16_t)(src->p3 * 10.0f);
                resp.entries[i].p4_x10 = (uint16_t)(src->p4 * 10.0f);
            }
        ...
        ESP_LOGI(TAG, "Sent preset list (%d pages, %d presets total)", total_pages, BLE_MAX_PRESETS);
        return 0;
    }
    ...
    switch (sub_cmd) {
        case BLE_CMD_LOAD_PRESET: {
            ...
            if (idx == PRESET_USER_DEFINED) {
                settings_set_user_defined();
                ESP_LOGI(TAG, "Switched to User Defined mode via BLE");
                ui_refresh_params();
                send_ack(BLE_MSG_CMD);
                break;
            }
        ...
        case BLE_CMD_SAVE_PRESET: {
            // Payload: [sub_cmd(1), index(1), name[20], p1_x10(2), t_x10(2), p2_x10(2),
            //           p3_x10(2), p4_x10(2)] = 32 bytes
            if (payload_len < 32) {
                send_nak(BLE_MSG_CMD, BLE_ERR_INVALID_RANGE);
                return 0;
            }
            ...
            uint16_t p3_x10  = (uint16_t)(payload[28] | (payload[29] << 8));
            uint16_t p4_x10  = (uint16_t)(payload[30] | (payload[31] << 8));

            float p1 = p1_x10 / 10.0f;
            float t  = t_x10  / 10.0f;
            float p2 = p2_x10 / 10.0f;
            float p3 = p3_x10 / 10.0f;
            ...
            settings_save_preset(idx, name, p1, t, p2, p3, p4);
